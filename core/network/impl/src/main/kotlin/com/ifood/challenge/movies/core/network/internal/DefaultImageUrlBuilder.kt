...
internal class DefaultImageUrlBuilder(
    ...
    override fun poster(path: String?, size: PosterSize): String? =
        path?.let { config.imageBaseUrl.trimEnd('/') + "/" + size.segment + ensureLeadingSlash(it) }

    override fun backdrop(path: String?, size: BackdropSize): String? =
        path?.let { config.imageBaseUrl.trimEnd('/') + "/" + size.segment + ensureLeadingSlash(it) }

    private fun ensureLeadingSlash(raw: String): String =
